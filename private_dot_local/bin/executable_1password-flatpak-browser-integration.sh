...
PACKAGE_LIST=$(flatpak list $FLATPAK_FLAGS --app --columns=application)
...
$MAYBE_SUDO_FLATPAK override $FLATPAK_FLAGS --talk-name=org.freedesktop.Flatpak "$FLATPAK_ID"
...
if [ "\${container-}" = flatpak ]; then
    flatpak-spawn --host $BIN_1P_BROWSER_SUPPORT "\$@"
else
    exec $BIN_1P_BROWSER_SUPPORT "\$@"
fi
EOF
...
if [[ "$BROWSER_TYPE" = "chromium" ]]; then
    ...
    BROWSERS_NOT_USING_MOZILLA=("org.mozilla.firefox" "io.gitlab.librewolf-community" "net.waterfox.waterfox" "app.zen_browser.zen")
    if [[ " ${BROWSERS_NOT_USING_MOZILLA[*]} " =~ [[:space:]]${FLATPAK_ID}[[:space:]] ]]; then
        ...
        if [[ "$CONTINUE" = "N" ]] || [[ "$CONTINUE" = "n" ]]; then
            ...
            $MAYBE_SUDO_FLATPAK override $FLATPAK_FLAGS --filesystem="$HOME/.mozilla/native-messaging-hosts" "$FLATPAK_ID"
